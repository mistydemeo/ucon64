...
int strhexdump(	char *str	//hexdump str from start for len
		,long start
		,long virtual_start	//if str is only a part of a bigger buffer you can write here the start number of the counter which is displayed left
		,long len
);
...
size_t quickfwrite(	const void *src	//same as fwrite but takes start and dest is a filename
			...
			,size_t len
			,char *dest
			,char *mode	//same like fopen() modes
);
