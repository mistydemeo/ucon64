...
unsigned char inp(unsigned long port);
void outp(unsigned long port, unsigned char data);
int par;
*/
#endif
#define sleep(32) snooze(1)
#endif
#include "smd.h"

/* from loader.asm */

unsigned char loader[0x100] =
{
...
uint16 lpt;// = 0x378;
uint8 block[0x4000];
char *opts[] = {"-bc", "-lc", "-bs", "-ls", "-ci", "-db", "-rc", NULL};
        ...
        if(strcmp(opts[i], argv[1]) == 0)
        {
            switch(i)
            {
                case 0: /* Backup cartridge */
                	#ifdef __BEOS__
                	par = open("/dev/misc/parnew", O_NONBLOCK|O_RDWR);
                	#endif
                    save_smd(argv[2]);
                    #ifdef __BEOS__
                    close(par);
...
{
    int timeout = 0x2000;
    int busy = (inportb(LPT_STATUS) >> 7) & 1;

    while((busy == state) && (timeout))
    {
        busy = (inportb(LPT_STATUS) >> 7) & 1;
        timeout -= 1;      
    }
...
void smd_send_byte(uint8 data)
{
    ...
    /* Send data to SMD */
    outportb(LPT_OUTPUT, data);

    /* Invert PC busy flag */
    outportb(LPT_CONTROL, inportb(LPT_CONTROL) ^ 1);
}
...
uint8 smd_recieve_byte(void)
{
    ...
    temp = (inportb(LPT_STATUS) & 0x78) >> 3;
    ...
    temp |= (inportb(LPT_STATUS) & 0x78) << 1;
...
/*
#ifdef __BEOS__
uint8 inp(uint32 port)
{
	IO_Tuple temp;
	
	temp.Port = port;

	ioctl(par, DRV_READ_IO_8, &temp, 0);
	
	return (temp.Data);
}


void outp(uint32 port, uint8 data)
{
	IO_Tuple temp;
	...
	ioctl(par, DRV_WRITE_IO_8, &temp, 0);
