...
static void MD4_Encode (unsigned char *output, UINT4 * input,
                        unsigned int len);
static void MD4_Decode (UINT4 * output, unsigned char *input,
                        unsigned int len);
...
MD4Update (MD4_CTX * context, unsigned char *input, unsigned int inputLen)
{
  ...
  if (inputLen >= partLen)
    {
      memcpy ((MD4_POINTER) & context->buffer[index], (MD4_POINTER) input,
              partLen);
  ...
  memcpy ((MD4_POINTER) & context->buffer[index], (MD4_POINTER) & input[i],
          inputLen - i);
}
...
static void
MD4_Transform (UINT4 * state, unsigned char *block)
{
...
//     a multiple of 4.
static void
MD4_Encode (unsigned char *output, UINT4 * input, unsigned int len)
{
...
//     a multiple of 4.
static void
MD4_Decode (UINT4 * output, unsigned char *input, unsigned int len)
{
  unsigned int i = 0, j = 0;
