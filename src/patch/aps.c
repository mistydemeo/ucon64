...
ReadN64Header (int Force)
{
	...
	{
		if (!n64aps_Quiet) { printf ("Incorrect Image\n"); fflush (stdout); }
		if (!Force)
		{
			fclose (n64aps_ORGFile);
			fclose (n64aps_APSFile);
...
ReadSizeHeader (char *File1)
{
	...
	if (OrigSize != APSOrigSize) // Do File Resize
	{
		...
		{
			t = 0;
			for (i=0;i<(APSOrigSize-OrigSize);i++) fputc (t,n64aps_ORGFile);
		}
	}
	...
	while (!Finished)
	{
		...
		else
		{
			...
			else
			{
				...
				for (i=0;i<len;i++) fputc (data,n64aps_ORGFile);
			}
		}
	}
}
...
WriteStdHeader (char *Desc)
{
	...
	fwrite (&n64caps_EncodingMethod,sizeof (n64caps_EncodingMethod),1,n64caps_APSFile);
...
WriteSizeHeader ()
{
	...
	if (ORGSize != NEWSize) n64caps_ChangeFound=n64caps_TRUE;
	...
	while (!Finished)
	{
		...
		while (i<NEWReadLen)
		{
			...
			case 2:
				...
				fwrite (NEWBuffer+(ChangedOffset),1,ChangedLen,n64caps_APSFile);
				State=0;
				break;
			}
		}
		...
		if (NEWReadLen==0) Finished=n64caps_TRUE;
		FilePos+=NEWReadLen;
	}
}
...
n64caps_main (int argc, char *argv[])
{
	...
	if (!n64caps_CheckFile (OutFile,"wb",n64caps_FALSE)) return(1);
...
int aps_usage(int argc,char *argv[])
{
printf("\
