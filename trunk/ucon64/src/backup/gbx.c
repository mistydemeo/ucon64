...
unsigned long newfsize(char *fname)
{
	struct stat sbuf;
		...
		if((i & 0xf) == 0xf) {
			...
			for(j = 0; j < 16; j++) {
				if(mix.buffer[(i & 0xfff0) + j] < 0x20
					|| mix.buffer[(i & 0xfff0) + j] > 0x80) printf(".");
				else
...
char spp_read_data(void)
{
	...
	set_data_read outportb(port_a, 2);	// nastb=1,nib_sel=0,ndstb=0,nwrite=1
...
{
	set_data_write outportb(ai, _ai);
}

void epp_set_ai_data(unsigned char _ai, unsigned char _data)
{
	epp_set_ai(_ai);
	set_data_write outportb(data, _data);
}

void set_ai(unsigned char _ai)
{
	set_data_write if(port_type)
		  spp_set_ai(_ai);
	else
		epp_set_ai(_ai);
}
...
//   set_data_read              // ninit=0, nWrite=1
	set_data_write write_data(d);	// out data
...
// ai[]=2 w control d7=rs,d1=xwe_en,d0=cs_en
	...
	set_data_write write_data(h);	// set rom bank value
...
void out_adr_data(unsigned long adr, unsigned char d)	// real address
{
	...
	set_data_write write_data(xh);	// set rom bank extend value
	set_adr(0x2000);			// write 2000:h
	set_data_write write_data(h);	// set rom bank value
//   outportb(data,h);          // set rom bank value
	set_ai_data(1, m);			// a[15..8]
...
//   disp_buffer(64);
	if(mix.buffer[0] == 0x89 && mix.buffer[1] == 0x15 && mix.buffer[0x10] == 'Q'
...
//       printf("^");
	...
	for(j = 0; j < 64; j++) {	// 16k bytes = 64 x 256 bytes
		...
		set_data_read for(i = 0; i < 256; i++) {	// page=256
...
//return(0);
		...
		set_ai(3);				// read/write data
		set_data_read for(i = 0; i < 128; i++) {	// page=128
			temp = read_data();
//          temp=inportb(data);
			if(temp != mix.buffer[idx + i]) {
