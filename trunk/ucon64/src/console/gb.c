...
gameboy_gbx (st_rominfo_t *rominfo)
{
  ...
  q_fcpy (ucon64.rom, 0, rominfo->buheader_len, ucon64_fbackup (NULL, buf), "wb");
...
gameboy_sgb (st_rominfo_t *rominfo)
{
  ...
  q_fwrite (ucon64.rom, 0, rominfo->buheader_len, ucon64_fbackup (NULL, buf), "wb");
...
gameboy_mgd (st_rominfo_t *rominfo)
{
  ...
  strcpy (buf, areupper (basename2 (ucon64.rom)) ? "GB" : "gb");
  strcat (buf, basename2 (ucon64.rom));
  if ((p = strrchr (buf, '.')))
    *p = 0;
  strcat (buf, "________");
