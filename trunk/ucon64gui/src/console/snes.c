/*
snes.c - Super Nintendo support for uCON64gui
...
#include "xpm/swc.xpm"
  ...
  htk_img (snes_96_xpm, 0, 0, 0, NULL);
  htk_img (snes2_96_xpm, 0, 0, 0, NULL);
  htk_img (snes3_96_xpm, 0, 0, 0, NULL);
  htk_img (swc_xpm, 0, 0, 0, NULL);

  ucon64gui_divider ();

  htk_input_submit ("Join", OPTION_S "j", "(" OPTION_S "j) join splitted ROM");

  htk_input_submit ("Split", OPTION_S "s",
                    "(" OPTION_S "s) split ROM into 4Mb parts (for backup unit(s) with fdd)");

  htk_ (" ");

  htk_input_submit ("Crack", OPTION_S "k", "(" OPTION_S "k) remove protection (crack)");

  htk_ (" ");

  htk_input_submit ("NTSC/PAL fix", OPTION_S "f", "(" OPTION_S "f) remove NTSC/PAL protection");
  ...
  htk_input_submit ("ColorCalc", OPTION_LONG_S "col",
  ...
  htk_input_submit ("SMC", OPTION_LONG_S "smc", "(" OPTION_LONG_S "smc) convert to Super Magicom/SMC");

  htk_input_submit ("FIG", OPTION_LONG_S "fig", "(" OPTION_LONG_S "fig) convert to *Pro Fighter(all)FIG");

  htk_input_submit ("SWC", OPTION_LONG_S "swc",
                    "(" OPTION_LONG_S "swc) convert to Super Wild Card(all)SWC");

  htk_input_submit ("MGD", OPTION_LONG_S "mgd",
                    "(" OPTION_LONG_S "mgd) convert to Multi GameMGD2/MGH/RAW");

  htk_input_submit ("GD3", OPTION_LONG_S "gd3",
                    "(" OPTION_LONG_S "gd3) convert to Professor SF(2) Game Doctor SF3/6/7");

  htk_ (" ");

  htk_input_submit ("Non-interleaved", OPTION_LONG_S "dint",
                    "(" OPTION_LONG_S "dint) convert ROM to non-interleaved format");

  htk_ (" ");

  htk_input_submit ("SlowROM", OPTION_S "l", "(" OPTION_S "l) convert to SlowROM");

  ucon64gui_spacer ();

  htk_ ("Convert SRAM to: ");

  htk_input_submit ("FIG", OPTION_LONG_S "figs",
                    "(" OPTION_LONG_S "figs) convert Snes9x/ZSNES *.srm (SRAM) to *Pro Fighter(all)FIG; $ROM=SRAM");

  htk_input_submit ("SWC", OPTION_LONG_S "swcs",
                    "(" OPTION_LONG_S "swcs) convert Snes9x/ZSNES *.srm (SRAM) to Super Wild Card(all)SWC; $ROM=SRAM");

  htk_input_submit ("UFO", OPTION_LONG_S "ufos",
                    "(" OPTION_LONG_S "ufos) convert Snes9x/ZSNES *.srm (SRAM) to Super UFO; $ROM=SRAM");

  htk_input_submit ("Snes9x/ZSNES", OPTION_LONG_S "stp",
